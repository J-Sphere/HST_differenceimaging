...
foreach my $file (@files){
    my ($errorflag,$target,$filter,$instrument,$detector,$propid,$dateobs,$timeobs, $fcnum, $ccdchip, $survey) = &GetFitsKeywords($file,"TARGNAME","FILTER","INSTRUME","DETECTOR","PROPOSID","DATE-OBS","TIME-OBS", "FCNUM","CCDCHIP", "SURVEY");
    if ($instrument ne "WFC3"){
    ...
    my $newnameroot1 ="${field}.$filter.$fcnum.CCD1.$propid.$file_short\_$mjd";
    my $newnameroot2 ="${field}.$filter.$fcnum.CCD2.$propid.$file_short\_$mjd";

    $newoutdir1 = "$outrootdir/$survey\_$filter/$fcnum";
    $newoutdir2 = "$outrootdir/$survey\_$filter/$fcnum";
